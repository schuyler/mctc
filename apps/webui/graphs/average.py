...
from apps.mctc.models import ReportCache, Report, Case
...
def create_average_for_case(case, field, length):
    ...
    if last_cache_date < now:
        for fld in ("muac", "weight", "height"):
            reports = {}        
            last = None            
            for report in Report.objects.filter(case=case):
                reports[report.entered_at.date()] = getattr(report, fld)
    
            # find the first
            first = Report.objects.filter(case=case).filter(entered_at__lte=last_cache_date).order_by("-entered_at")
            if first:
